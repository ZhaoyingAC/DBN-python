...
Author: Armstrong He
Time: 2017/7/25
Read specified attributes from HTRU_1 and save these attributes as pickle
"""
...
def save_attributes(dir_path):
    if not os.path.isfile('record.txt'):
        record = open('record.txt', 'w')
        record.close()
    record = open('record.txt', 'a')
    for child_path in os.listdir(dir_path):
        if "RFI" in child_path or "pulsars" in child_path and "tar.gz" not in child_path:
            file_name = child_path
            candidate_path = os.path.join(dir_path, child_path)
            all_attrs = list()
            ...
            for file_path in os.listdir(candidate_path):
                ...
                if "RFI" in file_name:
                    attrs.append(0)
                elif "pulsars" in file_name:
                    attrs.append(1)

                all_attrs.append(attrs)
            print("size of pulsar or rfi: ", len(all_attrs))
            if not os.path.exists("%s/save_attrs" % dir_path):
                os.mkdir("%s/save_attrs" % dir_path)
            with open("%s/save_attrs/%s.pkl" % (dir_path, file_name), "wb") as f:
                print("%s/save_attrs/%s.pkl" % (dir_path, file_name))
                date_time = str(datetime.now().strftime('%Y/%m/%d_%H:%M:%S'))
                record.write('%s: Saving %s.pkl\n'% (date_time, file_name))
                pickle.dump(all_attrs, f, -1)
                date_time = str(datetime.now().strftime('%Y/%m/%d_%H:%M:%S'))
                record.write('%s: Having saved %s.pkl\n' % (date_time, file_name))
            print("Having saved!")


if __name__=="__main__":
    path = "/home/ai/SKA/Data/HTRU_1/"
    save_attributes(path)
    # file = open("D:\\Arm\\BaiduNetdiskDownload\\MedlatTrainingData\\svae_sttrs\\pulsars.pkl", 'rb')
